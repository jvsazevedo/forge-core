...
impl MLP {
    fn new(builder: VarBuilder) -> Result<Self> {
        let ln1 = linear(INPUT_SIZE, 4, builder.pp("ln1"))?;
        let ln2 = linear(4, 2, builder.pp("ln2"))?;
        let ln3 = linear(2, RESULTS + 1, builder.pp("ln3"))?;
        Ok(Self { ln1, ln2, ln3 })
    }

    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let xs = self.ln1.forward(xs)?;
        let xs = xs.maximum(&(&xs * 0.01)?)?;
...
fn train(m: Dataset, dev: &Device) -> anyhow::Result<MLP> {
    let train_results = m.train_results.to_device(dev)?;
    let train_votes = m.train_votes.to_device(dev)?;

    let varmap = VarMap::new();
    let v_builder = VarBuilder::from_varmap(&varmap, DType::F32, dev);
    let model = MLP::new(v_builder.clone())?;
    ...
    let mut final_accuracy: f64 = 0.0;
    ...
    for epoch in 1..EPOCHS + 1 {
        ...
        let test_accuracy = sum_ok / test_results.dims1()? as f64;
        final_accuracy = 100. * test_accuracy;

        println!(
            "Epoch: {epoch:3} Train loss: {:8.5} Test accuracy: {:5.2}%",
            loss.to_dtype(DType::F64)?.to_scalar::<f64>()?,
            final_accuracy
        );
...
fn main() -> anyhow::Result<()> {
    ...
    let train_votes_tensor = Tensor::from_vec(
        ...
        (train_votes_vec.len() / INPUT_SIZE, INPUT_SIZE),
        &dev,
    )?
    .to_dtype(DType::F32)?;
    ...
    let train_results_tensor =
        Tensor::from_vec(train_results_vec, train_votes_vec.len() / INPUT_SIZE, &dev)?;

    let test_votes_vec: Vec<u32> = vec![13, 9, 8, 14, 3, 10];
    let test_votes_tensor = Tensor::from_vec(
        test_votes_vec.clone(),
        (test_votes_vec.len() / INPUT_SIZE, INPUT_SIZE),
        &dev,
    )?
    .to_dtype(DType::F32)?;
    ...
    let tensor_test_votes =
        Tensor::from_vec(real_world_votes.clone(), (1, INPUT_SIZE), &dev)?.to_dtype(DType::F32)?;
